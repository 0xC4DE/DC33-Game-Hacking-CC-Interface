-- !! CHANGEME !!
local puzzle_label = "puzzle"
local selected_puzzle = "1"
...
local puzzle_label = puzzle_select:addLabel():setText("Select Puzzle"):setPosition(3,1):setWidth("{self.text:len() + 1}")
local puzzle1_button = puzzle_select:addButton()
puzzle1_button:setText("1")
local button_width = puzzle1_button.text:len()+2
puzzle1_button:setSize(button_width, 1):setPosition(3, 2):setBackground(colors.green)
...
local function selectPuzzleButton(element) 
    puzzle_buttons[tonumber(selected_puzzle)]:setBackground(colors.gray)
    element:setBackground(colors.green)
    selected_puzzle = tonumber(element:getText())
end
...
for i, button in pairs(puzzle_buttons) do
