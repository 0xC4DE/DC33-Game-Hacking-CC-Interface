...
local computerId = 1006
local puzzle_label = "puzzle7"
...
local function receive_file(data)
    ...
    file.write(data)
    commands.exec(string.format("setblock ~ ~2 ~ computercraft:turtle_advanced[facing=south]{Fuel:50,ComputerId:%i}", computerId))
            ...
            if block ~= nil then
                if block["name"] ~= "minecraft:air" then
                    if block.state.lit then
                        ...
                        commands.exec("title @a subtitle \"Puzzle 7\"")
